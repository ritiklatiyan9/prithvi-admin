...
export const adminService = {
  ...
  listUsers: async (params: {
    ...
    const { data } = await apiClient.get<ApiSuccess<User[]>>("/admin/users", { params });
    return { items: data.data, meta: data.meta! };
  },

  updateUserRole: async (id: string, role: Role): Promise<User> => {
    const { data } = await apiClient.patch<ApiSuccess<User>>(`/admin/users/${id}/role`, { role });
    return data.data;
  },

  updateUserStatus: async (id: string, isActive: boolean): Promise<User> => {
    const { data } = await apiClient.patch<ApiSuccess<User>>(`/admin/users/${id}/status`, {
      isActive,
    });
    return data.data;
  },
