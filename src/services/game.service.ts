...
export const TTT_DIFFICULTIES: TttDifficulty[] = ["EASY", "MEDIUM", "HARD", "IMPOSSIBLE"];
...
export const TTT_SETTING_KEYS = {
  ...
  dailyLimit: "game.ttt.dailyLimit",
} as const;
...
export const gameService = {
  ...
  tttConfig: async (): Promise<TttConfig> => {
    const { data } = await apiClient.get<ApiSuccess<TttConfig>>("/game/ttt/config");
    return data.data;
  },
};
